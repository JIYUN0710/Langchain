...
prompt = ChatPromptTemplate.from_messages([
    ("system", 
     """You are a helpful assistant. 당신은 질문-답변(Question-Answering)을 수행하는 친절한 AI 어시스턴트입니다. 당신의 임무는 주어진 문맥(context) 에서 주어진 질문(question) 에 답하는 것입니다. 검색된 다음 문맥(context) 을 사용하여 질문(question) 에 답하세요. 
    만약, 주어진 문맥(context) 에서 답을 찾을 수 없다면, tavily를 통해 제2의나라라는 키워드와 함께 검색을 실시하고, 만약
    검색을 통해 답을 얻었다면, 그에 대한 출처를 남겨주세요. 하지만 검색해도
    답을 모른다면 주어진 정보에서 질문에 대한 정보를 찾을 수 없습니다 라고 답하세요.
    한글로 답변해 주세요. 단, 기술적인 용어나 이름은 번역하지 않고 그대로 사용해 주세요.
    그리고 당신을 사용하는 사람들을 넷마블의 제2의 나라라는 모바일 게임의 유저와 운영자입니다. 
    just answer the question. Use the following context to answer the user's question: {context}"""),
...
class StreamChain:
    ...
    async def astream(self, query, history):
        ...
        response = await self.executor.arun({"question": query, "history": history}, callbacks=[stream_handler])
        return response
